...
@Slf4j
public class VerifyClearanceRecordServiceAdapter_2_3_0 extends VerifyClearanceRecordService {
  ...
  protected Map<Long, VerifiedClearanceRecordInfo> buildVerifiedClearanceRecordInfoMap(
      ...
      @NonNull final VerifyReport report) {
    ...
    if (crList.size() > 1) {
      ...
      while (!clearanceRecordMap.isEmpty()) {
        final long previousCo = currentCo - 1;
        final ClearanceRecord previousRecord = clearanceRecordMap.get(previousCo);
        ...
        verifiedClearanceRecordInfoMap.put(previousCo, VerifiedClearanceRecordInfo.builder()
                                                                                  .pass(pass)
                                                                                  .clearanceRecord(previousRecord)
                                                                                  .build());
        currentCo--;
        currentRecord = previousRecord;
      }
      log.debug("buildVerifiedClearanceRecordInfoMap(), reportListSize={}", reportList.size());
      report.setVerifyClearanceRecordReportList(reportList);
    }
    log.debug("buildVerifiedClearanceRecordInfoMap() end, verifiedClearanceRecordInfoMap={}",
  ...
  protected VerifyChainHashResult verifyChainHash(@NonNull final ClearanceRecord currentRecord,
     ...
     * Hash.sha3("")
     */
    byte[] concatByteArray = ByteUtils.concatByteArray(HashUtils.hex2byte(previousRecord.getRootHash()),
        Numeric.toBytesPadded(BigInteger.valueOf(previousRecord.getClearanceOrder()), 32));
